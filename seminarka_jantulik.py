...
def bit_to_mb(n):
    if isinstance(list, tuple):  # kontrola zda input neni list nebo tuple
        mbList = []
        for x in range(len(n)):
            tmp = n[x]
            tmp = (1023.0 ** -2) * tmp  # prevedeni konkretnich hodnot v listu/tuplu na MB
            mbList.append(tmp)
        return mbList
    else:
        return (1023.0 ** -2) * n  # prevedni samotne hodnoty
